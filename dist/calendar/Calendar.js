...
export const Calendar = /*#__PURE__*/React.memo( /*#__PURE__*/React.forwardRef((props, ref) => {
  ...
  const onUserInput = event => {
    ...
    try {
      let val = rawValue !== undefined ? rawValue : '';
      let day = '',
        time = '';
      ...
      if (day.length === 8) {
        let dd = '',
          ...
          hh = '',
          mi = '';
        dd = day.substring(0, 2);
        mm = day.substring(2, 4);
        yyyy = parseInt(day.substring(4, 8)) - diffYear;
        day = yyyy + '-' + mm + '-' + dd;
        if (time.length === 4) {
          hh = time.substring(0, 2);
          mi = time.substring(2, 4);
          time = 'T' + hh + ':' + mi + ':00Z';
          newDate = new Date(day + time);
          let timezoneOffset = newDate.getTimezoneOffset() / 60;
          newDate.setHours(newDate.getHours() + timezoneOffset);
        } else {
          newDate = new Date(day);
        }
        if (props.showTime && time.length === 10 || props.showTime === false) {
          updateModel(event, newDate);
          updateViewDate(event, newDate.length ? newDate[0] : newDate);
        }
      }
    } catch (err) {
      //invalid date
        ...
        if (isSingleSelection()) {
          ...
          if (formattedValue !== '') {
            ...
            if (_date.length === 3) {
              ...
              formattedValue = monthMap[_date[0]] || _date[0];
            }
  ...
  const createTitleMonthElement = month => {
    ...
    if (props.monthNavigator && props.view !== 'month') {
      ...
      const displayedMonthOptions = monthNames.map((month, index) => (!isInMinYear(viewDate) || index >= props.minDate.getMonth()) && (!isInMaxYear(viewDate) || index <= props.maxDate.getMonth()) ? {
        label: month,
        value: index,
        index
      } : null).filter(option => !!option);
      const displayedMonthNames = displayedMonthOptions.map(option => option.label);
      const content = /*#__PURE__*/React.createElement("select", {
        className: "p-datepicker-month",
        onChange: e => onMonthDropdownChange(e, e.target.value),
        value: viewMonth
      }, displayedMonthOptions.map(option => /*#__PURE__*/React.createElement("option", {
        key: option.label,
        value: option.value
      }, option.label)));
  ...
  const createTitleYearElement = () => {
    if (props.yearNavigator) {
      ...
      for (let i = yearStart; i <= yearEnd; i++) {
        yearOptions.push(i + diffYear);
      }
      const viewDate = getViewDate();
      const viewYear = viewDate.getFullYear() + diffYear;
      const displayedYearNames = yearOptions.filter(year => !(props.minDate && props.minDate.getFullYear() + diffYear > year) && !(props.maxDate && props.maxDate.getFullYear() + diffYear < year));
      const content = /*#__PURE__*/React.createElement("select", {
  ...
  const createTitleDecadeElement = () => {
    ...
    if (currentView === 'year') {
      ...
      }, props.decadeTemplate ? props.decadeTemplate(years) : /*#__PURE__*/React.createElement("span", null, `${yearPickerValues()[0] + diffYear} - ${yearPickerValues()[yearPickerValues().length - 1] + diffYear}`));
    }
    return null;
  };
  ...
  const createDayNames = weekDays => {
    const dayNames = weekDays.map((weekDay, index) => /*#__PURE__*/React.createElement("th", {
      key: `${weekDay}-${index}`,
      scope: "col"
  ...
  const createDates = (monthMetaData, groupIndex) => {
    return monthMetaData.dates.map((weekDates, index) => /*#__PURE__*/React.createElement("tr", {
      key: index
  ...
  const createMonthViewMonth = index => {
    const className = classNames('p-monthpicker-month', {
      ...
      'p-disabled': !isSelectable(1, index, currentYear)
    });
  ...
  const createHourPicker = () => {
    ...
    const headerHour = props.locale === 'th' ? 'ชั่วโมง' : 'Hour';
    return /*#__PURE__*/React.createElement("div", {
      className: "p-hour-picker"
    }, props.showHeaderTime && /*#__PURE__*/React.createElement("span", {
      className: "text-base"
    }, headerHour), /*#__PURE__*/React.createElement("button", {
      type: "button",
      className: "p-link",
  ...
  const createMinutePicker = () => {
    ...
    const headerMinute = props.locale === 'th' ? 'นาที' : 'Minute';
    return /*#__PURE__*/React.createElement("div", {
      className: "p-minute-picker"
    }, props.showHeaderTime && /*#__PURE__*/React.createElement("span", {
      className: "text-base"
    }, headerMinute), /*#__PURE__*/React.createElement("button", {
      type: "button",
      className: "p-link",
  ...
  const createSecondPicker = () => {
    if (props.showSeconds) {
      ...
      const headerSecond = props.locale === 'th' ? 'วินาที' : 'Second';
      return /*#__PURE__*/React.createElement("div", {
        className: "p-second-picker"
      }, props.showHeaderTime && /*#__PURE__*/React.createElement("span", {
        className: "text-base"
      }, headerSecond), /*#__PURE__*/React.createElement("button", {
        type: "button",
        className: "p-link",
